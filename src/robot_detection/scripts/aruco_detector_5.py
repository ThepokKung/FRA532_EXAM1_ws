...
class ArucoDocker(Node):
    def __init__(self):
        ...
        self.reverse_threshold = 0.8  # เริ่มถอยหลังเมื่อใกล้เกินนี้
        self.Kp_linear = 0.1
        self.Kp_angular_position = 0.15
        self.Kp_angular_yaw = 0.3
        self.center_tolerance = 10      # พิกเซล
        self.yaw_tolerance = 0.1        # เรเดียน (~5.7°)
        self.reverse_speed = -0.1       # ความเร็วถอยหลัง
        ...
        try:
            ...
            if ids is not None:
                ...
                for i in range(len(ids)):
                    ...
                    # คำนวณมุม Yaw
                    rotation_matrix, _ = cv2.Rodrigues(rvecs[i][0])  # แก้ไขให้รับ rvec ที่ถูกต้อง
                    yaw = self.rotation_matrix_to_yaw(rotation_matrix)
                    
                    # ควบคุมการเคลื่อนที่
                    if distance < self.reverse_threshold:
                        # อยู่ในระยะถอยหลัง
                        if (abs(horizontal_error) > self.center_tolerance) or (abs(yaw) > self.yaw_tolerance):
                            cmd_vel.linear.x = self.reverse_speed
                            cmd_vel.angular.z = -self.Kp_angular_position*(horizontal_error/(self.image_width//2)) - self.Kp_angular_yaw*yaw
                            status = "REVERSING & ALIGNING"
                        else:
                    ...
                    cv2.putText(cv_image, 
                               f"Yaw: {np.degrees(yaw):.1f}° | Dist: {distance:.2f}m", 
